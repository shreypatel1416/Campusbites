...
app.post("/restaurant/create-rest", async (req, res) => {
  ...
  try {
    ...
    console.log("Created restaurant with ID:", newRestId);
...
app.get("/dineat/:restId/tables", async (req, res) => {
  try {
    const restId = parseInt(req.params.restId);
    console.log(`Attempting to fetch tables for restaurant ID: ${restId}`);
    
    const filter = { restId: restId };
    console.log(`Searching for restaurant with filter:`, filter);
    
    let restaurant = await Restaurant.findOne(filter);
    console.log(`Database query result:`, restaurant);
    
    if (!restaurant) {
      console.log(`No restaurant found with ID: ${restId}. Creating a new one.`);
      ...
      console.log(`New restaurant object created:`, restaurant);
      
      try {
        await restaurant.save();
        console.log(`Created new restaurant with ID: ${restId}`);
      } catch (saveError) {
        console.error(`Error saving restaurant:`, saveError);
        return res.status(500).render("error.ejs", { 
          message: "Server Error",
          error: { status: 500, stack: "An error occurred while creating the restaurant." }
        });
      }
    }
    ...
    res.status(500).render("error.ejs", { 
      message: "Server Error",
      error: { status: 500, stack: "An error occurred while fetching tables." }
    });
  }
});
...
app.get("/dineat/:restId/table/:tableNumber/create-order", async (req, res) => {
  try {
    ...
    console.log(`Creating order for restaurant ID: ${restId}, table: ${tableNum}`);
    ...
    // Check if the restaurant has any tables
    if (!restaurant.tables || restaurant.tables.length === 0) {
      console.log(`Restaurant ${restId} has no tables. Creating a default table.`);
      
      // Add a default table
      restaurant.tables = [{ number: 1 }];
      await restaurant.save();
      console.log(`Added default table to restaurant ${restId}`);
      
      // If the requested table number is not 1, redirect to table 1
      if (tableNum !== 1) {
        console.log(`Redirecting from table ${tableNum} to table 1`);
        return res.redirect(`/dineat/${restId}/table/1/create-order`);
      }
    } else {
      // Check if the requested table exists
      const tableExists = restaurant.tables.some(table => table.number === tableNum);
      if (!tableExists) {
        console.log(`Table ${tableNum} does not exist in restaurant ${restId}`);
        return res.status(404).render("error.ejs", { 
          message: "Table not found",
          error: { status: 404, stack: `Table ${tableNum} does not exist in this restaurant.` }
        });
      }
    }
    ...
    console.log(`Restaurant found: ${restaurant.name}`);
    const menus = restaurant.menu;
    console.log(`Number of menu items: ${menus.length}`);
    ...
    res.status(500).render("error.ejs", { 
      message: "Server Error",
      error: { status: 500, stack: "An error occurred while creating order." }
    });
  }
});
...
app.get("/about-us", (req, res) => {
  res.render("restaurant/about-us", { restId: 0 });
});

app.get("/contact-us", (req, res) => {
  res.render("restaurant/contact-us", { restId: 0 });
});
